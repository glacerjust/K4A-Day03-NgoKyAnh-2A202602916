        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            if not obs_data:
                ...
                if obs_data.get("status") == "SUCCESS":
                    if "data" in obs_data:
                        d = obs_data["data"]
                        if "applied_position" in d:
                            final_answer = (
                                f"Kết quả tra cứu ứng viên {obs_data.get('candidate_id', '')} ({d.get('full_name', '')}): "
                                f"Vị trí: {d.get('applied_position', '')}, Kinh nghiệm: {d.get('experience', '')}, "
                                f"Kỹ năng: {', '.join(d.get('skills', []))}, Email: {d.get('email', '')}, "
                                f"Trạng thái: {d.get('status', '')}."
                            )
                        elif "requirements" in d:
                            final_answer = (
                                f"Tiêu chí tuyển dụng cho vị trí {obs_data.get('job_title', '')}: "
                                f"Phòng ban: {d.get('department', '')}, Cấp độ: {d.get('level', '')}, "
                                f"Lương: {d.get('salary_range', '')}. Yêu cầu: {', '.join(d.get('requirements', []))}."
                            )
                        else:
                            final_answer = f"Lấy dữ liệu thành công: {json.dumps(d, ensure_ascii=False)}"
...
if __name__ == "__main__":
    print("==========================================================")
    print("🏢 HR AI ASSISTANT - RECRUITMENT & CV SCREENING")
    print("==========================================================")
    ...
    if "--interactive" in sys.argv:
        ...
        print("   - Câu hỏi chung: 'Quy trình tuyển dụng của công ty như thế nào?'")
        print("   - Tra cứu tiêu chí: 'Hãy tra cứu tiêu chí tuyển dụng cho vị trí Data Scientist'")
        print("   - Đặt lịch hẹn: 'Đặt lịch phỏng vấn cho UV2024001 vào lúc 14:00 ngày 20/10/2026'")
